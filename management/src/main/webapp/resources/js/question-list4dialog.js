...
var question_list = {
    ...
    redirectUrl: function (newparam) {
        ...
        document.location.href = document.getElementsByTagName('base')[0].href
            + util.getCurrentRole() + '/question/question-list/filterdialog-' + paramurl;
    },
    ...
    preventDuplicateQuestion: function () {
        ...
        for (var l = 0; l < questionIdList.length; l++) {
            questionIdArray.push($(questionIdList[l]).text());
        }


        console.log(questionIdList.length);

        var questionIdListSource = $("#question-list tbody tr .question-id");

        for (var i = 0; i < questionIdListSource.length; i++) {
            var questionId = $(questionIdListSource[i]).text();
            if ($.inArray(questionId, questionIdArray) != -1) {
                $(questionIdListSource[i]).parent().find("input").attr("disabled", "disabled");
            }
