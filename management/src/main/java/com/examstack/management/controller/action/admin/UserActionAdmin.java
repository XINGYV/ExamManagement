...
public class UserActionAdmin {
    ...
    public @ResponseBody
    Message updateTeacher(@RequestBody User user) {
        UserInfo userInfo = (UserInfo) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        user.setCreateTime(new Date());
    ...
    public @ResponseBody
    Message changeTeacherStatus(Model model, @PathVariable("userId") int userId, @PathVariable("enabled") boolean enabled) {
