...
//			this.bindAddQustionToPaper();
    ...
    loadAnswerSheet: function loadAnswerSheet() {
        $.ajax({
            ...
            type: "GET",
            async: false,
            url: "student/get-answersheet/" + $("#hist-id").val(),
...
//				$(targetQuestion).focus();
            ...
            request.done(function (message, tst, jqXHR) {
                ...
                if (message.result == "success") {
                    ...
                    util.success("阅卷完成！", function () {
                        window.location.replace(document.getElementsByTagName('base')[0].href + 'admin/exam/exam-student-list/' + $("#exam-id").val());
                    });
                } else {
    ...
    genrateAnswerSheet: function genrateAnswerSheet() {
        ...
        for (var i = 0; i < questions.length; i++) {
            ...
            answerSheetItem.comment = $(questions[i]).find(".raw-answer-comment").text();
